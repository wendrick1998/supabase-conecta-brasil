...
export const AutomationSidebar: React.FC<AutomationSidebarProps> = ({
  ...
  const renderBlockTemplates = (blocks: {
    ...
    return blocks.map(block => (
      <div 
        ...
        tabIndex={0}
        onKeyDown={(e) => e.key === 'Enter' && onBlockClick(block.id)}
      >
  ...
  return (
    <div 
      className={`border-r border-vendah-purple/20 bg-surface flex flex-col ${collapsed ? 'w-[60px]' : 'w-[250px]'}`}
      role="region" 
      aria-label="Blocos disponíveis para automação"
    >
        ...
        <div className="p-3">
          {collapsed ? (
            ...
            <Tabs defaultValue="triggers">
              <TabsList className="grid grid-cols-3 mb-4" aria-label="Categorias de blocos">
          ...
          {!collapsed && (
            <div className="mt-8">
              <Button 
                ...
                aria-label="Ver templates de automação"
              >
                Ver Templates
              </Button>
            </div>
